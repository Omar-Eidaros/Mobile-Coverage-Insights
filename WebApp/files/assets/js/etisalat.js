...
function responseManipulation(response) {
  for (let i = 0; i < Object.size(response['measurement']); i++) {
    ...
    var signal_level;
    if (Math.abs(obj["properties"].status) <= 60) {
    ...
    else if ( Math.abs(obj["properties"].status) <= 80 ) {
      signal_level = "Good"
      x = L.circleMarker([obj["geometry"]["coordinates"][0], obj["geometry"]["coordinates"][1]],
       { radius: 23, color: "yellow", weight: 3, fillOpacity: 0.5 }).addTo(mymap);
    } 
    else if(Math.abs(obj["properties"].status) <= 100){
      signal_level = "Poor"
...
function DataShow() {
  ...
  xhr.open("GET", "https://immense-journey-36861.herokuapp.com/measurment/DML/getAllMeasurements/etisalat");
...
document.getElementById("search-btn").addEventListener('click',function(){
  ...
  xhr.open("GET", "http://api.positionstack.com/v1/forward?access_key=305db552c8ef2a0a37c5e8280165bc51&query="+ document.getElementById("address-search").value);
  xhr.onreadystatechange = function () {
    if (xhr.readyState == 4) {
      if (xhr.status == 200) {
        response = JSON.parse(xhr.responseText);
